...
##
##@export var README: String = "IMPORTANT: MAKE SURE TO ASSIGN 'left' 'right' 'jump' 'dash' 'up' 'down' 'roll' 'latch' 'twirl' 'run' in the project settings input map. Usage tips. 1. Hover over each toggle and variable to read what it does and to make sure nothing bugs"
##
...
##var max_rope_length = 300.0
##var rope_firing_speed = 400.0
...
##func _on_rope_collision(position, collider):
	...
	##print("Rope attached to ", collider.name, " at position ", position)
...
#@export var segment_length := 5.0  # Length of each collision segment
#@export var max_segments := 4 #Testing this with a short rope rn
#
